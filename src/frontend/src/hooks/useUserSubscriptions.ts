...
export function useUserSubscriptions({ identity, authMethod, statusFilter, prev, take }: UseUserSubscriptionsOpts) {
  const key = identity ? identity.getPrincipal().toText() : authMethod === 'plug' ? 'plug' : 'none';
  return useQuery({
    queryKey: ['userSubscriptions', key, statusFilter, prev?.toString()],
    queryFn: async (): Promise<Subscription[]> => {
      ...
      const f: [] | [UserSubscriptionsFilter] = statusFilter
        ? [{ status: [statusFilter], subscriptions: [], services: [], products: [], subaccounts: [] }]
        : [];
